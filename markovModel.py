...
class FitnessLandscape:
	...
	def drawLandscape(self):
		...
		vmin = np.min(fits)
		vmax= np.max(fits)
		peak=[convertGenotypeToInt(gen)  for gen in self.getPeaks()]
		ecolors =np.array(['w']*self.genotypeLength()**2)
		ecolors[peak] = 'r'
		nx.draw(G,pos=pos, node_color=fits,ax=ax,node_size=500,edge_color='grey',edgecolors= ecolors,cmap=plt.cm.Blues)
